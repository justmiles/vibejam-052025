  ...
  <v-app>
    ...
    <v-main>
      <v-container fluid>
        <v-alert
          ...
          closable
        >
          <h3 class="text-h6 mb-2">How to Use VibeJam LLM Magic Button:</h3>
          <ol class="pl-4">
            <li class="mb-1"><strong>Wait for Initialization:</strong> The AI model and WASM tools will load when the app starts. You'll see progress indicators. This might take a moment, especially on first use as model files are downloaded.</li>
            <li class="mb-1"><strong>Enter Your Text:</strong> Type or paste the text you want the AI to work with into the "Input" area.</li>
            <li class="mb-1"><strong>Select a Template:</strong> Choose a processing template from the dropdown (e.g., Summarize, Regex Generator). This guides the AI on what to do.</li>
            <li class="mb-1"><strong>Click "Do Magic":</strong> The LLM will process your text based on the selected template.</li>
            <li class="mb-1"><strong>View Output:</strong> The AI's response will appear in the "Output" area.</li>
            <li class="mb-1"><strong>LLM Tools:</strong> The AI can use built-in Go tools (like Base64 encoding/decoding or math operations) if its task requires them. You might see "TOOL_CALL" in the console logs if it uses one.</li>
            <li><strong>Test Go Tools:</strong> You can directly test the Go WASM tools (Add, Multiply, Base64) via the "Toggle Go Tools Test" option in the navigation menu (☰).</li>
          </ol>
        </v-alert>
        ...
        <v-row>
          <v-col cols="12" md="6">
            ...
            <div v-if="llmModelLoadingProgress && !llmModelLoaded" class="mb-4">
              ...
              <v-progress-linear
                :model-value="llmModelLoadingProgress.percentage"
                color="amber"
                height="20"
                striped
              >
            ...
            <div
              v-if="llmModelLoaded && !finalizingLoad"
              class="mb-2"
              style="color: green"
            >
          ...
          <v-col cols="12" md="6">
            ...
            <v-card outlined min-height="200px">
              <v-card-text>
                ...
                <div
                  v-else
                  class="text-center grey--text lighten-1--text"
        ...
        <v-row v-if="showWasmTestArea">
          <v-col cols="12">
            ...
            <p v-if="wasmError" style="color: red">
              Error loading WASM tools: {{ wasmError }}
            </p>
            <div v-if="wasmLoaded">
              ...
              <v-row>
                <v-col cols="12" md="6">
                  <v-card outlined class="pa-4 mb-4">
                    <h3>Go Add (Tool)</h3>
                    <v-text-field
                      v-model.number="addArg1"
                      label="Arg 1"
                      type="number"
                      density="compact"
                    ></v-text-field>
                    <v-text-field
                      v-model.number="addArg2"
                      label="Arg 2"
                      type="number"
                      density="compact"
                    ></v-text-field>
                    <v-btn @click="performGoAdd" color="info" class="mt-2"
                      >Add</v-btn
                    >
                    <p class="mt-2" v-if="addResult !== null">
                  ...
                  <v-card outlined class="pa-4">
                    ...
                    ></v-text-field>
                    <v-btn
                      @click="performGoBase64Encode"
                      color="info"
                      class="mt-2"
                      >Encode</v-btn
                    >
                    <p class="mt-2" v-if="base64EncodedResult !== null">
                      Encoded: <strong>{{ base64EncodedResult }}</strong>
                    </p>
                  </v-card>
                </v-col>
                <v-col cols="12" md="6">
                  <v-card outlined class="pa-4 mb-4">
                    ...
                    ></v-text-field>
                    <v-text-field
                      v-model.number="multiplyArg2"
                      label="Arg 2"
                      type="number"
                      step="0.1"
                      density="compact"
                    ></v-text-field>
                    <v-btn @click="performGoMultiply" color="info" class="mt-2"
                      >Multiply</v-btn
                    >
                    <p class="mt-2" v-if="multiplyResult !== null">
                      Result: <strong>{{ multiplyResult }}</strong>
                    </p>
                  </v-card>
                  <v-card outlined class="pa-4">
                    <h3>Go Base64 Decode (Tool)</h3>
                    <v-text-field
                      v-model="base64ToDecodeInput"
                      label="Base64 to Decode"
                      density="compact"
                    ></v-text-field>
                    <v-btn
                      @click="performGoBase64Decode"
                      color="info"
                      class="mt-2"
                      >Decode</v-btn
                    >
                    <p class="mt-2" v-if="base64DecodedResult !== null">
                      Decoded: <strong>{{ base64DecodedResult }}</strong>
                    </p>
                    <p
                      class="mt-2"
                      v-if="base64DecodeError !== null"
                      style="color: red"
                    >
    ...
    <v-footer app color="primary" dark>
      <span class="white--text"
        >&copy; {{ new Date().getFullYear() }} VibeJam</span
      >
    </v-footer>
  </v-app>
</template>
...
onMounted(async () => {
  try {
    ...
    wasmError.value =
      error.message || "Unknown error during WASM tools loading.";
  }
  ...
  progressInterval = setInterval(() => {
    if (simulatedProgress.value < 30) {
      ...
      const currentActualPercentage =
        llmModelLoadingProgress.value?.actualPercentage || 0;
      llmModelLoadingProgress.value = {
        ...llmModelLoadingProgress.value,
  ...
  setLLMInitProgressCallback((progress) => {
    ...
    if (
      progress.progress * 100 >= simulatedProgress.value &&
      progressInterval
    ) {
  ...
  try {
    ...
    if (isLLMInitialized()) {
      ...
      progressInterval = null;
      llmModelLoadingProgress.value = {
        text: `Model ${getLLMLoadedModelId()} loaded! Finalizing...`,
        percentage: 100,
      };
    ...
    progressInterval = null;
    llmError.value =
      error.message || "Unknown error during LLM Engine initialization.";
    llmModelLoadingProgress.value = {
      text: `Error: ${error.message || "Failed to load model."}`,
      percentage: 0,
    };
  }
});
...
const canProcess = computed(() => {
  return (
    inputAreaRef.value?.getText()?.length > 0 &&
...
const performGoBase64Encode = async () => {
  if (!wasmLoaded.value) {
    base64EncodedResult.value = "WASM tools not loaded.";
    return;
  }
  try {
...
const performGoBase64Decode = async () => {
  if (!wasmLoaded.value) {
    base64DecodedResult.value = "WASM tools not loaded.";
    return;
  }
